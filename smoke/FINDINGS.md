...
| SMK-012 | P3 | i18n | UI | Applications filter renders the **raw key `common.all`** instead of "All": the key is missing from message files, and next-intl `t()` returns the key (truthy) so the `\|\| 'All'` fallback is defeated. **Any missing key leaks as raw text this way.** Confirmed on NGO/Donor/Reviewer. | Open — add key + **systematic i18n key-coverage audit** before launch (fix+rebuild as one batch) |
| SMK-013 | P2 | NGO onboarding / directory | UI | NGO `/network/directory` hangs on **"Loading…" forever** — the members API returns 403 for a non-admin NGO and the page has no error/empty state, so it spins indefinitely. Found by driving the browser as fatima. | Open — handle the 403 with an empty/permission state (fix+rebuild) |
| SMK-015 | **P2** | Tenant isolation | API/UI | Donor `/verification` (and `/organizations/search`) **leak orgs from other tenants** (Proximate, Saxansaxo) — `api_get_all_verifications` runs `Organization.query.filter(~org_type.in_(['donor','reviewer']))` with **no network scoping**. A Kuja donor sees non-Kuja NGOs. | Open — scope to `get_current_network_id()` (same class as the X-Network-Override seal) |
...
- **Stale-persona header on in-tab re-login**: swapping session via `fetch` without a full navigation leaves the old avatar/name in the header (backend identity is correct). Resolves on real navigation. Cosmetic; worth a note for support.
...
- **Latent: criterion without a stable `key` couples in the scorer** — `scores[c.key]`; if a criterion ever lacks `key`, all such criteria write to `scores[undefined]`. Wizard always assigns keys, so low severity, but a defensive index-fallback would harden it.
- **Response-key mismatch**: the submit validator accepts `criterion_{idx}` (or `id`) keys, while the reviewer scorer renders NGO responses by `c.key`. Cosmetic today (responses still store/submit), but the NGO's answer may not render next to the matching criterion if keys and indexes diverge.
- **overall_score weight_map keyed by `id` not `key`** (reviews.py ~1008): when criteria have `key` but no `id`, weights fall back to 1 each — correct only when weights are equal. Worth aligning to `key`.
