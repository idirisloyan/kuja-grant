...
- **Capacity assessment lives in the Trust app.** Today there is **no in-UI hand-off** from the Grant app to the Trust app; the "real" C4C assessment is completed by signing in to the Trust app directly. The Grant app also has its own in-app assessment/passport (legacy). Grant application submission is **not** blocked by the Trust profile — it's a soft nudge. (See the Capacity & Trust tab, rows TRU-010 to TRU-012.)
