...
Most items DONE 2026-07-17 in `82752d8d1`:
- ~~Approved-activity editing UI~~ — DONE: "Approved activities" card on
  the round page (partner + name + budget lines USD, add/delete).
- ~~Per-item request-changes~~ — DONE: OB "Flag" per photo/video/file
  with a note; shows on the partner token page (EN/AR); clears on
  resubmit; audited.
- ~~Video streaming~~ — DONE: evidence files stream with HTTP Range
  (206) so videos/voice notes scrub.
- ~~Exchange rate on spend~~ — DONE: OB sets SDG-per-USD rate on the
