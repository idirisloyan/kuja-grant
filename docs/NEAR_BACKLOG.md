...
- [ ] Register message templates: `endorsement_invite`,
      `report_ack`, `disbursement_notify`.
- [ ] Backend send helper + wire into three call sites (endorsement
      create → invite, report submit → ack, disbursement record →
      notify).
- [ ] Env vars: `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_ACCESS_TOKEN`,
      `WHATSAPP_BUSINESS_ACCOUNT_ID`.
- [ ] Fallback path: keep the existing `wa.me/?text=` manual share
      button working alongside the API-driven auto-nudges.
