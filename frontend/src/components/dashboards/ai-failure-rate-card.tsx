...
export function AiFailureRateCard() {
  const [data, setData] = useState<Resp | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const state = { cancelled: false };
    api.get<Resp>('/api/dashboard/ai-failure-rate').then((r) => {
    ...
    <Card className={high ? 'border-rose-200' : undefined}>
      ...
      <CardContent className="text-sm space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          {data.failed} of {data.total} AI calls failed in the last {data.window_hours}h.
        </p>
      </CardContent>
    </Card>
  );
