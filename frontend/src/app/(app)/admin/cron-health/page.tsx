...
function timeAgo(
  ...
  if (!iso) return t('cron_health.never_ran');
  const d = new Date(iso).getTime();
  const ms = Date.now() - d;
  const s = Math.round(ms / 1000);
  if (s < 60) return t('cron_health.seconds_ago', { n: s });
  if (s < 3600) return t('cron_health.minutes_ago', { n: Math.round(s / 60) });
  if (s < 86400) return t('cron_health.hours_ago', { n: Math.round(s / 3600) });
  return t('cron_health.days_ago', { n: Math.round(s / 86400) });
}
...
export default function CronHealthPage() {
  ...
  return (
    <PageShell>
      <PageHeader
        title={t('cron_health.title')}
        icon={Clock}
        subtitle={t('cron_health.subtitle')}
        secondaryAction={<FreshnessStamp loadedAt={loadedAt} label={t('cron_health.loaded')} />}
      />
      <PageMain>
        {loading && (
        ...
        {data && (
          <>
            <div className="grid gap-3 sm:grid-cols-3 mb-4">
              <Card className="p-4">
                <div className="text-xs text-muted-foreground">{t('cron_health.fresh')}</div>
              ...
              <Card className={cn('p-4', data.summary.overdue > 0 ? 'border-amber-300' : '')}>
                <div className="text-xs text-muted-foreground">{t('cron_health.overdue')}</div>
              ...
              <Card className={cn('p-4', data.summary.never > 0 ? 'border-rose-300' : '')}>
                <div className="text-xs text-muted-foreground">{t('cron_health.never')}</div>
            ...
            {totalBad > 0 && (
              <div className="mb-3 rounded-md border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900 inline-flex items-center gap-2">
                ...
                {t(
                  totalBad === 1
                    ? 'cron_health.jobs_need_attention_one'
                    : 'cron_health.jobs_need_attention_other',
                  { n: totalBad },
                )}
              </div>
            )}
              ...
              {data.crons.map((c) => (
                <Card key={c.name} className="p-4">
                  ...
                  <div className="text-[11px] text-muted-foreground space-x-2">
                    <span>{t('cron_health.cadence', { n: c.cadence_hours })}</span>
                    <span>{t('cron_health.last', { when: timeAgo(c.last_run_at, t) })}</span>
                    {c.last_duration_ms != null && <span>({c.last_duration_ms}ms)</span>}
                    {c.last_success === false && (
                      <span className="text-rose-700 font-semibold">{t('cron_health.failed')}</span>
                    )}
                  </div>
                  {c.last_summary && (
