...
export default function ProximateEndorseInvitePage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!r.ok || !body.success) {
        setError(body.error || t('proximate.endorse_invite.submit_failed'));
      } else {
        setSubmitted(true);
      }
    } catch {
      setError(t('proximate.endorse_invite.submit_failed'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-6">
        <div className="max-w-md w-full text-center space-y-4">
          ...
          <h1 className="text-xl font-semibold">{t('proximate.endorse_invite.thank_you')}</h1>
          <p className="text-sm text-muted-foreground">
            {t('proximate.endorse_invite.recorded', { name: data?.partner?.name || '' })}
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-lg mx-auto space-y-6">
        <div className="text-center space-y-2">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">
            {t('proximate.endorse_invite.header')}
          </p>
          <h1 className="text-2xl font-bold">
            {t('proximate.endorse_invite.greeting', { name: inv.invitee_name })}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t('proximate.endorse_invite.intro')}
          </p>
        ...
        <div className="bg-muted/40 rounded-lg p-4 space-y-2">
          <p className="text-xs uppercase tracking-wider text-muted-foreground">
            {t('proximate.endorse_invite.the_ngo')}
          </p>
          ...
          {inv.note && (
            <p className="text-xs italic border-t pt-2 mt-2 text-muted-foreground">
              {t('proximate.endorse_invite.note', { note: inv.note })}
            </p>
          )}
        </div>
        ...
        <div className="space-y-4">
          {[
            { key: 'q1', value: q1, setter: setQ1, label: t('proximate.endorse.q1'), hint: t('proximate.endorse_invite.q1_hint') },
            { key: 'q2', value: q2, setter: setQ2, label: t('proximate.endorse.q2'), hint: t('proximate.endorse_invite.q2_hint') },
            { key: 'q3', value: q3, setter: setQ3, label: t('proximate.endorse.q3'), hint: t('proximate.endorse_invite.q3_hint') },
          ].map((q) => (
        ...
        >
          ...
          {t('proximate.endorse_invite.submit')}
        </button>
        ...
        <p className="text-[10px] text-center text-muted-foreground">
          {t('proximate.endorse_invite.footer')}
        </p>
      </div>
    </div>
  );
